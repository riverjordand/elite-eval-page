...
const LandingMediaStrip = () => {
  ...
  return (
    <section className="py-10 md:py-14 lg:py-16 bg-background overflow-hidden">
      <div className="container mx-auto px-4 mb-6 md:mb-8">
        <div className="text-center">
          <h2 className="font-bebas text-2xl md:text-3xl lg:text-4xl text-foreground uppercase tracking-tight mb-1">
            Training In Motion
          </h2>
          <p className="font-oswald text-xs md:text-sm text-muted-foreground">
      ...
      {/* Videos row */}
      <div className="mb-3 overflow-hidden" ref={videoRef}>
        <div className="flex gap-2 md:gap-3">
          {videos.map((video, index) => (
            <div key={index} className="flex-none w-44 md:w-56 lg:w-72 aspect-video rounded-lg overflow-hidden">
              <video
                src={video}
                autoPlay
                loop
      ...
      <div className="overflow-hidden" ref={photoRef}>
        <div className="flex gap-2 md:gap-3">
          {photos.map((photo, index) => (
            <div key={index} className="flex-none w-32 md:w-44 lg:w-52 aspect-square rounded-lg overflow-hidden">
              <img 
                src={photo} 
                alt={`Action ${index + 1}`}
                className="w-full h-full object-cover"
