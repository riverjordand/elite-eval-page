    ...
    <section className="relative py-20 md:py-28 lg:py-36 overflow-hidden">
      ...
      <div className="container relative mx-auto px-6 lg:px-16">
        ...
        <div className="text-center mb-12 lg:mb-16">
          <div className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-bebas text-sm px-5 py-2 mb-6 uppercase tracking-[0.2em]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 mb-12 lg:mb-16">
          {includes.map((item, index) => (
            <div key={index} className="bg-background/50 backdrop-blur border border-border/40 p-6 hover:border-primary/40 transition-colors group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <Check className="w-5 h-5 text-primary flex-shrink-0" />
                  <span className="font-bebas text-lg text-foreground uppercase">{item.item}</span>
                </div>
                <span className="font-bebas text-lg text-primary">{item.value}</span>
              </div>
              <p className="font-oswald text-sm text-muted-foreground pl-8">{item.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="text-center">
          <div className="inline-block bg-background/70 backdrop-blur border border-border/40 p-8 lg:p-12">
            ...
            <Button 
              size="lg"
              onClick={onCtaClick}
              className="group bg-primary hover:bg-primary/90 text-primary-foreground font-bebas uppercase tracking-[0.15em] text-xl lg:text-2xl px-10 lg:px-14 py-6 lg:py-8 h-auto mb-6"
              style={{ boxShadow: '0 0 50px hsl(271 81% 56% / 0.4)' }}
            >
              Book My Free Evaluation
              <ArrowRight className="ml-3 w-6 h-6 lg:w-7 lg:h-7 group-hover:translate-x-1 transition-transform" />
            </Button>
            
            <div className="flex flex-wrap items-center justify-center gap-6 text-muted-foreground">
              <div className="flex items-center gap-2">
                <Timer className="w-4 h-4 text-primary" />
                <span className="font-oswald text-sm">60-Minute Session</span>
              </div>
              <div className="flex items-center gap-2">
                <Shield className="w-4 h-4 text-primary" />
                <span className="font-oswald text-sm">No Obligation</span>
              </div>
            </div>
          </div>
        </div>
