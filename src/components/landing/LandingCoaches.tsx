        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
            {coaches.map((coach, index) => (
              <div 
                key={index}
                className="bg-card border-2 border-border rounded-xl p-4 md:p-8 text-center hover:border-primary/50 transition-all duration-300 animate-fade-in"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                {/* Coach Avatar Placeholder */}
                <div className="w-16 h-16 md:w-24 md:h-24 bg-primary/10 rounded-full mx-auto mb-4 md:mb-6 flex items-center justify-center">
                  <span className="text-2xl md:text-4xl font-bebas text-primary">
