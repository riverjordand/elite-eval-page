...
const LandingFacilityShowcase = ({
  title,
  subtitle,
  areas
}: LandingFacilityShowcaseProps) => {
  return <>
          ...
          <div className="text-center mb-8 md:mb-16">
            ...
            <p className="text-sm sm:text-base md:text-xl text-muted-foreground font-oswald max-w-2xl mx-auto px-2">
              ...
              width: 'max-content'
                  ...
                  {areas.map((area, index) => <div key={index} className="group flex-shrink-0 w-[85vw] bg-card border-[3px] border-border rounded-xl overflow-hidden shadow-lg hover:border-primary/60 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 animate-fade-in snap-center" style={{
                animationDelay: `${index * 100}ms`
                      ...
                      <div className="w-full aspect-video bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center relative overflow-hidden">
                        {area.image ? <img src={area.image} alt={`${area.title} at LPA`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" /> : <>
                      ...
                      {/* Content */}
                      <div className="p-4">
                        <h3 className="text-xl font-bebas font-black uppercase mb-2 text-primary text-center">
                          {area.title}
                        </h3>
                        <p className="text-xs text-foreground font-oswald mb-4 leading-relaxed text-center">
                          {area.description}
                        </p>
                        <ul className="space-y-2">
                          {area.specs.map((spec, specIndex) => <li key={specIndex} className="flex items-start gap-2 text-center">
                              <div className="w-1 h-1 bg-primary rounded-full mt-1.5 flex-shrink-0" />
                              <span className="text-muted-foreground font-oswald text-xs text-center">{spec}</span>
                            </li>)}
                        </ul>
                      </div>
                    </div>)}
            ...
            {areas.map((area, index) => <div key={index} className="group bg-card border-[3px] border-border rounded-xl overflow-hidden shadow-lg hover:border-primary/60 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 animate-fade-in flex-shrink-0 w-[420px]" style={{
              animationDelay: `${index * 100}ms`
            }}>
                {/* Image */}
                <div className="w-full aspect-video bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center relative overflow-hidden">
                ...
                <div className="p-8">
                  ...
                  <ul className="space-y-3">
                    {area.specs.map((spec, specIndex) => <li key={specIndex} className="flex items-start gap-3">
