    ...
    <section className="relative py-fluid-section overflow-hidden">
      ...
      <div className="container relative mx-auto px-fluid-container-px">
        ...
        <div className="max-w-4xl mx-auto">
          <Accordion type="single" collapsible className="space-y-fluid-xs">
            {faqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`faq-${index}`}
                className="group border border-border/30 rounded-xl lg:rounded-2xl px-fluid-md bg-gradient-to-br from-card/60 to-card/30 backdrop-blur-sm hover:border-primary/40 hover:bg-card/50 transition-all duration-300"
              >
                <AccordionTrigger className="font-bebas text-fluid-xl text-foreground uppercase hover:no-underline py-fluid-sm group-hover:text-primary transition-colors">
                  {faq.q}
                </AccordionTrigger>
                <AccordionContent className="font-oswald text-fluid-base text-muted-foreground pb-fluid-sm">
                  {faq.a}
                </AccordionContent>
              </AccordionItem>
            ))}
